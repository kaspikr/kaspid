...
// 2. A wrapper that implements isKaspidMessage_Payload, having a single field pointing to the concrete command
//    bearing the name of the command with `KaspidMessage_` prefix and `Request` suffix
...
// generateKaspidMessage generates a wrapped KaspidMessage with the given `commandValue`
func generateKaspidMessage(commandValue reflect.Value, commandDesc *commandDescription) (*protowire.KaspidMessage, error) {
	commandWrapper := reflect.New(commandDesc.typeof)
	unwrapCommandValue(commandWrapper).Set(commandValue)

	kaspidMessage := reflect.New(reflect.TypeOf(protowire.KaspidMessage{}))
	kaspidMessage.Elem().FieldByName("Payload").Set(commandWrapper)
	return kaspidMessage.Interface().(*protowire.KaspidMessage), nil
}
