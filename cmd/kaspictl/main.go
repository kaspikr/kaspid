...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		kaspidMessage, err := client.Post(&protowire.KaspidMessage{Payload: &protowire.KaspidMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := kaspidMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	kaspidMessage := &protowire.KaspidMessage{}
	err := protojson.Unmarshal([]byte(response), kaspidMessage)
	if err != nil {
	...
	return marshalOptions.Format(kaspidMessage)
}
