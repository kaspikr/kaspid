...
func commandLoop(argsChan <-chan []string) ([]commandFailure, error) {
	...
	dataDirectoryPath, err := common.TempDir("kaspidsanity-kaspid-datadir")
	if err != nil {
	...
	for args := range argsChan {
		...
		cmd := exec.Command("kaspid", args...)
		cmd.Stdout = common.NewLogWriter(log, logger.LevelTrace, "KASPID-STDOUT")
		cmd.Stderr = common.NewLogWriter(log, logger.LevelWarn, "KASPID-STDERR")
