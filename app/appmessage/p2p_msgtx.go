...
// Outpoint defines a kaspi data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new kaspi transaction outpoint point with the
...
// TxIn defines a kaspi transaction input.
type TxIn struct {
...
// NewTxIn returns a new kaspi transaction input with the provided
...
// TxOut defines a kaspi transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new kaspi transaction output with the provided
...
// MsgTx implements the Message interface and represents a kaspi tx message.
