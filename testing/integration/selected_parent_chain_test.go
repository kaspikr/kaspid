...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of kaspid instances
	kaspid1, kaspid2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := kaspid1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In kaspid2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, kaspid2)
	}

	// Connect the two kaspids. This should trigger sync
	// between the two nodes
	connect(t, kaspid1, kaspid2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := kaspid1.rpcClient.GetVirtualSelectedParentChainFromBlock(
