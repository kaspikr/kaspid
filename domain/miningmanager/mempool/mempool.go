...
import (
	...
	miningmanagermodel "github.com/kaspikr/kaspid/domain/miningmanager/model"
)
...
func (mp *mempool) BlockCandidateTransactions() []*externalapi.DomainTransaction {
	...
	for _, tx := range readyTxs {
		if len(tx.Outputs) > len(tx.Inputs) {
			...
			if !hasCoinbaseInput && numExtraOuts > 2 && tx.Fee < uint64(numExtraOuts)*constants.SompiPerKaspi {
				log.Debugf("Filtered spam tx %s", consensushashing.TransactionID(tx))
				continue
			}

			if hasCoinbaseInput || tx.Fee > uint64(numExtraOuts)*constants.SompiPerKaspi {
