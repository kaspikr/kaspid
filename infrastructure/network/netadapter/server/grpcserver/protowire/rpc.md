...
Clients are expected to build RequestMessages and wrap them in KaspidMessage. (see messages.proto)

Having received a RequestMessage, (wrapped in a KaspidMessage) the RPC server will respond with a
ResponseMessage (likewise wrapped in a KaspidMessage) respective to the original RequestMessage.
...
GetCurrentNetworkRequestMessage requests the network kaspid is currently running against.
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| payAddress | [string](#string) |  | Which kaspi address should the coinbase block reward transaction pay into |
...
| ----- | ---- | ----- | ----------- |
| block | [RpcBlock](#protowire.RpcBlock) |  |  |
| isSynced | [bool](#bool) |  | Whether kaspid thinks that it&#39;s synced. Callers are discouraged (but not forbidden) from solving blocks when kaspid is not synced. That is because when kaspid isn&#39;t in sync with the rest of the network there&#39;s a high chance the block will never be accepted, thus the solving effort would have been wasted. |
| error | [RPCError](#protowire.RPCError) |  |  |
...
GetPeerAddressesRequestMessage requests the list of known kaspid addresses in the
...
currently connected to this kaspid.
...
| isOutbound | [bool](#bool) |  | Whether this kaspid initiated the connection |
| timeOffset | [int64](#int64) |  |  |
| userAgent | [string](#string) |  |  |
| advertisedProtocolVersion | [uint32](#uint32) |  | The protocol version that this peer claims to support |
| timeConnected | [int64](#int64) |  | The timestamp of when this peer connected to this kaspid |
...
AddPeerRequestMessage adds a peer to kaspid&#39;s outgoing connection list.
This will, in most cases, result in kaspid connecting to said peer.
...
parent chain from some startHash to this kaspid&#39;s current virtual
...
GetBlockCountRequestMessage requests the current number of blocks in this kaspid.
...
ShutDownRequestMessage shuts down this kaspid.
...
This call is only available when this kaspid was started with `--utxoindex`
...
GetUtxosByAddressesRequestMessage requests all current UTXOs for the given kaspid addresses
